...
class ODB_Handler:
    ...
    def write_params(self, results: list) -> bool:
        ...
        try:
            res_list = [(measurement_id, chidx, v, i, ts, status) for (chidx, v, i, ts, status) in results]
        ...
        with self.con:
            try:
                self.con.executemany(
                    "INSERT INTO data(measurement_id, channel, voltage, current, t, status) VALUES(?, ?, ?, ?, ?)", res_list
                )
