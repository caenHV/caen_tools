...
class ReducerControl(Script):
    ...
    async def exec_function(self):
        ...
        waiting_time = max(0, self.shared_parameters["repeat_every"] - self.shared_parameters["reducing_period"])
        ...
        logging.debug("ReducerControl: %s %s", self.relax.target_voltage, self.relax.voltage_modifier)
        ...
        self.relax.target_voltage, self.relax.voltage_modifier = relax_params[0], relax_params[1]
        logging.error("VALS %s %s", relax.target_voltage, relax.voltage_modifier)
        if self.interlock_status:
            logging.debug("ReducerControl: Interlock is up. No needs to restore voltage")
            self.form_answer(Codes.OK)
            self.send_mchs(True)
        else:
