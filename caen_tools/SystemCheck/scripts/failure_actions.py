...
async def reduce_voltage(hc: HealthControl, status: CheckStatus) -> None:
    ...
    reduce_voltage = await hc.cli.query(
        PreparedReceipts.set_voltage(
            hc.SENDER, target * hc.shared_parameters["soft_reduce_mod"]
        )
    )
...
async def check_and_increase(hc: HealthControl):
    ...
    if (
        ...
        logging.warning(
            "Too many emergency downs (%s) in last %s seconds. Autopilot will not be restarted. Send DownVoltage.",
            n_consecutive_downs,
            hc.shared_parameters["allowed_down_window"],
        )
        await hc.cli.query(PreparedReceipts.down(hc.SENDER))
        return
    if hc.shared_parameters["reduced"] is not None and (
        time.time() - hc.shared_parameters["reduced"]
        > hc.shared_parameters["reduce_period"]
    ):
        hc.shared_parameters["reduced"] = None
...
async def check_and_restart(hc: HealthControl):
    ...
    logging.debug(
        ...
        hc.shared_parameters["auto_restart_after"],
    )
    if hc.shared_parameters["last_down"] is not None and (
        time.time() - hc.shared_parameters["last_down"]
        > hc.shared_parameters["auto_restart_after"]
    ):
        await hc.cli.query(PreparedReceipts.reset_device(hc.SENDER))
        hc.shared_parameters["last_down"] = None
