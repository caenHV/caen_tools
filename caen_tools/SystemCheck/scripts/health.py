...
class HealthControl(Script):
    ...
    def perform_checks(self, params_dict: dict) -> bool:
        ...
        good_status = self.__check_ch_status(params_dict) and self.__check_currents(params_dict)
        return good_status
    ...
    async def failure_actions(self) -> None:
        ...
        down_voltage = await self.cli.query(
            PreparedReceipts.down(self.SENDER)
        )
