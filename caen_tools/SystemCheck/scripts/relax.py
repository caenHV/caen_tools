...
class RelaxControl(Script):
    ...
    async def exec_function(self):
        ...
        receipt = await self.cli.query(
            PreparedReceipts.get_voltage(self.SENDER)
        )
        if isinstance(receipt.response, ReceiptResponseError):
        ...
        if interlock and not isclose(current_voltage, reduced_voltage, abs_tol=1e-4):
            logging.info("Different current (%.3f) and reduced (%.3f) voltages. Set %.3f", 
                current_voltage, reduced_voltage, reduced_voltage)
            await self.set_voltage(reduced_voltage)
        elif not interlock and not isclose(current_voltage, target_voltage, abs_tol=1e-4):
            logging.info("Different current (%.3f) and target (%.3f) voltages. Set %.3f", 
                current_voltage, target_voltage, target_voltage)
